"use strict";
const colorBoxCanvas = document.getElementById('colorBoxCanvas');
if (!colorBoxCanvas) {
    console.error('ColorBox canvas not found');
}
const colorBoxCtx = colorBoxCanvas.getContext('2d');
const colors = [
...
function drawColorWheel(canvas, size = 150) {
    ...
    while (angle < 360) {
        ...
        context.arc(radius, radius, radius, degreesToRadians(angle), degreesToRadians(360));
        context.closePath();
        context.fill();
        angle++;
    }
